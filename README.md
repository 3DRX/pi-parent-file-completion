...
```bash
pi -e /absolute/path/to/pi-parent-file-completion/src/index.ts
```
...
ln -s /absolute/path/to/pi-parent-file-completion/src/index.ts \
...
This project is a Pi package. You can also load the whole local package by adding it to Pi settings or installing it as a local package:

```bash
pi install /absolute/path/to/pi-parent-file-completion
```

Project-local install from a project that should use it:

```bash
pi install -l /absolute/path/to/pi-parent-file-completion
```
